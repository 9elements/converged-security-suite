...
func run(testGroup string, tests []*test.Test, preset *test.PreSet) bool {
	...
	for index := range tests {
		...
		fmt.Fprintf(&s, "%02d - ", index)
		fmt.Fprintf(&s, "%-40s: ", a.Bold(tests[index].Name))

		if tests[index].Status == test.NotImplemented {
			fmt.Fprintf(&s, "%-20s", a.Bold(a.Yellow(tests[index].Status)))
		...
		case test.ResultPass:
			fmt.Fprintf(&s, "%-20s", a.Bold(a.Green(tests[index].Result)))
		default:
			fmt.Fprintf(&s, "%-20s", a.Bold(a.Red(tests[index].Result)))
			result = false
			if tests[index].ErrorText != "" {
				fmt.Fprintf(&s, " (%s)", tests[index].ErrorText)
			} else if len(tests[index].ErrorText) == 0 && tests[index].Result == test.ResultFail {
				fmt.Fprintf(&s, " %s", "(No error text given)")
			}
		}
		log.Infof("%s", s.String())
