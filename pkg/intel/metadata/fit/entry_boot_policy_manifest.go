...
import (
	...
	bg "github.com/9elements/converged-security-suite/v2/pkg/provisioning/bootguard/bootpolicy"
)
...
func (entry *EntryBootPolicyManifestRecord) ParseDataBG() (*bg.Manifest, error) {
	var bpManifest bg.Manifest
	_, err := bpManifest.ReadFrom(bytes.NewReader(entry.GetDataBytes()))
	if err != nil {
		return nil, fmt.Errorf("failed to parse KeyManifest, err: %v", err)
	}
