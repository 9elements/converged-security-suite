...
func (MeasureVideoImageInterpreter) Actions(ctx context.Context, s *types.State) types.Actions {
	return measureToTPMEachRangeSeparately(ctx, s, pcr.ID(0), amddata.BIOSDirectoryEntries(amdbiosimage.DirectoryLevelAll, manifest.VideoInterpreterEntry), tpmeventlog.EV_EFI_PLATFORM_FIRMWARE_BLOB, "VideoInterpreter")
}
